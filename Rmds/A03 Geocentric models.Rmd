...
```         
The first line is the likelyhood.
yi ∼ Normal(μ, σ) 
```
...
```         
Pr(μ,σ|y)=Pr(y|μ,σ)*Pr(μ)*Pr(σ)/integral(integral(Pr(y|μ,σ)*Pr(μ)*Pr(σ)dμdσ))
```
...
```         
Yes, I'd reduce the sd of the normal distribution of the slope to make sure it's always positive
b=dnorm(5/2000, 1/2000)
```
...
```         
sigma is therefore never higher than 8. That was the case already so I won't change it. That also would make me consider the a prior which I have right now as dunif (100,200)
I'd change that to dunif(140,180)
```
...
# and for each weight in weight.seq
mu <- link( m4.3 , data=data.frame(weight=weight.seq) )
mub <- link( m4.3b , data=data.frame(weight=weight.seq) )

# use type="n" to hide raw data
plot( mu ~ mub )
# loop over samples and plot each mu value
# for ( i in 1:100 ){
#         points( weight.seq , mu[i,] , pch=16 , col="red" )
#         points( weight.seq , mub[i,] , pch=16 , col="blue" )
# }
```
...
```         
The number of knots changes the shape a lot while changing the sigma doesn't have a big influence.
```
...
for (i in 1:length(individuals)){
        mu = post$a + post$b * ( individuals[i] - xbar )
        mu.mean <- mean(mu)
        pi_mu= PI(mu, prob = 0.89)
        print(c(round(i,digits = 0),round(individuals[i], digits=2),
                mu.mean,pi_mu))
}
...
```         
For every 10 units of weight, the expected increase in heigth is 10*b=27 units of heigth.
```
...
# for each sample from posterior
# and for each weight in weight.seq
mu <- link( m4h2a , data=data.frame(weight=weight.seq) )
# use type="n" to hide raw data
plot( height ~ weight , d1 , type="n" )
# loop over samples and plot each mu value
for ( i in 1:100 )
  points( weight.seq , mu[i,] , pch=16 , col=col.alpha(rangi2,0.1) )
# summarize the distribution of mu
...
# plot raw data
# fading out points to make line and interval more visible
plot( height ~ weight , data=d1 , col=col.alpha(rangi2,0.5) )
# plot the MAP line, aka the mean mu for each weight
lines( weight.seq , mu.mean )
# plot a shaded region for 89% PI
shade( mu.PI , weight.seq )
...
```         
The model does an alright job between 10 and 30 units of weight but fails completely outside of those bounds. the data looks very non linear and the data after 30 units of weight flattens out which forces the model to show a lower slope than the data.
```
...
(b) Begin with this plot: plot( height \~ weight , data=Howell1 ). Then use samples from the quadratic approximate posterior of the model in (a) to superimpose on the plot: (1) the predicted mean height as a function of weight, (2) the 97% interval for the mean, and (3) the 97% interval for predicted heights.
    ...
    # define sequence of weights to compute predictions for
    # these values will be on the horizontal axis
    weight.seq <- seq( from=1 , to=60 , by=1 )
    # use link to compute mu
    # for each sample from posterior
    # and for each weight in weight.seq
    mu <- link( m4h3a , data=data.frame(logweigth=log(weight.seq)) )
...
The data in data(bangladesh) are 1934 women from the 1989 Bangladesh Fertility Survey. For each woman, we know which district she lived in, her number of living.children, her age.centered, whether she lived in an urban center,and finally whether or not she used contraception (use.contraception). In the previous homework, you estimated the effect of surviving children on contraceptive use. Now I want you to repeat that model, but allowing the effect of children to vary by district and to be correlated with the other effects that vary by district. That is, use a model with correlated varying effects, like in lecture. Compare the posterior predictions to the previous model. How do the inferences change, if at all?
