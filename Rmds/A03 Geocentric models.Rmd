...
# define sequence of weights to compute predictions for
# these values will be on the horizontal axis
weight.seq <- seq( from=4 , to=60 , by=1 )
# use link to compute mu
# for each sample from posterior
# and for each weight in weight.seq
mu <- link( m4h4a , data=data.frame(weight_s=weight.seq,
...
data_cherry_full <- data_cherry[!is.na(data_cherry$doy) & !is.na(data_cherry$temp) ,]
```
...
m4h5a <- quap(
  alist(
    ...
    b ~ dlnorm( 0 , 2 ),
    sigma ~ dunif( 0 , 50 )
    ) , data=data_cherry_full )

temp.seq <- seq( from=min(data_cherry_full$temp) , 
                 to=max(data_cherry_full$temp) , 
                 by=0.01 )

mu <- link( m4h5a , data=data.frame(temp=temp.seq) )
plot( doy ~ temp , data=data_cherry_full )
for ( i in 1:100 )
...
plot( doy ~ temp , data=data_cherry_full , col=col.alpha(rangi2,0.5) )
# plot the MAP line, aka the mean mu for each weight
...
```{r}
# polynomial
data_cherry_full$temp_2 <- data_cherry_full$temp^2
m4h5b <- quap(
  alist(
    doy ~ dnorm( mu , sigma ) ,
    mu <- a + b1*temp+b2*temp_2 ,
    a ~ dnorm( 90 , 20 ) ,
    b1 ~ dlnorm( 0 , 1 ) ,
    b2 ~ dnorm( 0 , 1 ) ,
    sigma ~ dunif( 0 , 50 )
...
mu <- link( m4h5b , data=data.frame(temp=temp.seq,
                                    temp_2=temp.seq^2) )
plot( doy ~ temp , data=data_cherry_full )
for ( i in 1:100 )
  points( temp.seq , mu[i,] , pch=16 , col=col.alpha(rangi2,0.1) )
...
```{r}
num_knots <- 10
knot_list <- quantile( data_cherry_full$temp , 
                       probs=seq(0,1,length.out=num_knots) )
library(splines)
B <- bs(data_cherry_full$temp,
        knots=knot_list[-c(1,num_knots)] ,
        degree=3 , intercept=TRUE )
m4h5c <- quap(
  alist(
    ...
    w ~ dnorm(20,10),
    sigma ~ dexp(1)
    ), data=list( D=data_cherry_full$doy , B=B ) ,
  start=list( w=rep( 0 , ncol(B) ) 
              ) 
  )
...
mu <- link( m4h5c )
mu_PI <- apply(mu,2,PI,0.97)
plot( data_cherry_full$temp , data_cherry_full$doy , 
      col=col.alpha(rangi2,0.3) , pch=16 )
shade( mu_PI , data_cherry_full$temp , col=col.alpha("black",0.5) )
```
...
b <- rlnorm( N , -1 , 1 )
plot( NULL , 
      xlim=range(d$age) , ylim=c(100,180) ,
